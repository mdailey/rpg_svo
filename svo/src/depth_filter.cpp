...
void DepthFilter::removeKeyframe(FramePtr frame)
{
  ...
  std::list<Seed,Eigen::aligned_allocator<std::list<Seed> > >::iterator it=seeds_.begin();
  size_t n_removed = 0;
  while(it!=seeds_.end())
  {
...
void DepthFilter::getSeedsCopy(const FramePtr& frame, std::list<Seed,Eigen::aligned_allocator<std::list<Seed> > >& seeds)
{
  lock_t lock(seeds_mut_);
  for(std::list<Seed,Eigen::aligned_allocator<std::list<Seed> > >::iterator it=seeds_.begin(); it!=seeds_.end(); ++it)
  {
    if (it->ftr->frame == frame.get())
      seeds.push_back(*it);
  }
