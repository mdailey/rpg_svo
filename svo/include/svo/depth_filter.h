...
public:
  ...
  void getSeedsCopy(const FramePtr& frame, std::list<Seed,Eigen::aligned_allocator<std::list<Seed> > >& seeds);

  /// Return a reference to the seeds. This is NOT THREAD SAFE!
  std::list<Seed,Eigen::aligned_allocator<std::list<Seed> > >& getSeeds() { return seeds_; }
...
protected:
  ...
  std::list<Seed,Eigen::aligned_allocator<std::list<Seed> > > seeds_;
